...
export const CommissionWalletBalance =()=>{
    const {GetBalance,getAllCommisionStats} = useHttpHook()
    ...
    return  <div className="mt-4 lg:mt-10 relative lg:h-40">
            ...
            <div className="flex-grow" >
                <div className="text-[#FFFFFFD9] font-light flex item-center gap-3">
                <span className="w-[170px] lg:w-auto">Total Withdrawals</span>
                <button
                ...
                <div className="text-[#FFD983] text-[12px] lg:text-[14px]">Pending Approval: {MaskBalance((balance?.total_pending_withdrawal || "0.00"),!showBalance)}<span></span></div>
            </div>
            <div className="lg:me-20 flex items-center">
                {String(stats?.balance) !== "0.00" && <Link 
                ...
                >
                 <span className="lg:text-[14px] text-[12px]"> Withdraw (NGN{stats?.balance?.toLocaleString() || "0"})</span>
